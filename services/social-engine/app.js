...
const db = require('./database'); // Point to our new SQLite adapter
...
// --- SOCIAL ENGINE STARTUP ---
const PORT = process.env.PORT || 3003;
app.listen(PORT, () => {
    console.log(`✅ Social Engine (Kapota) online on port ${PORT}`);
});
...
app.post('/post/create', upload.single('media'), async (req, res) => {
    ...
    try {
        ...
        db.query(insertPostQuery, [userId, content || null, mediaUrl], (err, result) => {
            if (err) {
            ...
            if (mediaUrl && finalMediaType) {
                const insertMediaQuery = `
                    ...
                    VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                `;
                db.query(insertMediaQuery, [postId, mediaUrl, finalMediaType], (err2) => {
                    if (err2) {
                        console.error('Error inserting media:', err2);
                    }
                    res.redirect('/dashboard');
                });
            } else {
        ...
        db.query(
            ...
            (err, users) => {
                ...
                db.query(
                    ...
                    [profileUserId],
                    (err2, followerCountResult) => {
                        const followerCount = (followerCountResult && followerCountResult[0]) ? followerCountResult[0].follower_count : 0;

                        // Fetch following count
                        db.query(
                            'SELECT COUNT(*) AS following_count FROM followers WHERE follower_id = ?',
                            [profileUserId],
                            (err3, followingCountResult) => {
                                const followingCount = (followingCountResult && followingCountResult[0]) ? followingCountResult[0].following_count : 0;
                                ...
                                db.query(
                                    ...
                                    (err4, isFollowingResult) => {
                                        const isFollowing = (isFollowingResult && isFollowingResult[0]) ? isFollowingResult[0].is_following > 0 : false;
                                        ...
                                        db.query(
                                            ...
                                            (err5, posts) => {
                                                if (err5) {
                                                    console.error('Error fetching posts:', err5);
                                                    posts = [];
                                                }

                                                res.render('profile', {
                                                    profileOwner: userDetails,
                                                    posts: posts || [],
                                                    followerCount,
                                                    followingCount,
                                                    isFollowing,
